...
call "%CMAKE_CMD%" --install build_win --strip
echo Test CMAKE
cd build_win
call "%CTEST_CMD%" -C Debug --output-on-failure
cd ..

echo Running Test Executable

call .\publish_win\bin\utility_tests.exe
call .\publish_win\bin\inventory_lib_test_tests.exe
call .\publish_win\bin\InventoryApp.exe
